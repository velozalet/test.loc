...
require_once "twitteroauth/autoload.php"; //путь к скачанной библиотеке twitteroauth в корневой директории ПЛАГИНА(скачивать тут:https://twitteroauth.com/)
use Abraham\TwitterOAuth\TwitterOAuth;

// Settings keys_(получение ключей.В данном случае мы их просто используем готовые,с имеющегося у Вити Аккаунта на Twitter):
define('CONSUMER_KEY','8zx0iDI1URlyDp84Fg3lydMUm');
define('CONSUMER_SECRET','q2DCR4TEzf3OxT7pB7DwUBfQLZ3wleavTM1yN1KEUBGbbsAa5U');
define('ACCESS_TOKEN','89649557-JUdPM5iSFVnayMfrphv6iRMVLVtYXExlLD1lM2tp1');
define('ACCESS_TOKEN_SECRET','UtQ7DUooabMP9Awd6bpfVhZUcKryC7NHFHXSQMeDzJoCb');
...
function table_create() {
    ...
    // Получаем выборку Твиттов по заданным координатом,заданному радиусу и в указанном кол-ве:
    ...
    $content = $connection->get("search/tweets", array("count" => 5, //Поиск Твиттов в кол-ве("count" => 5),кот.мы вытягиваем к себе в ПЛАГИН
        "q" => get_option('theme_twitt'), // тема на которую будет поиск твиттов
        "geocode" => get_option('width_twitt_gmap').','.get_option('length_twitt_gmap').','.get_option('radius_twitt_gmap').'mi', // гео-координаты в виде: 45.50735445050955,-122.67608642578125,10mi // Здесь,через get_option мы эти значения берем из БД,а в нее они попадают через поля ввода(настройки) в Админке
        "lang" => "en", // язык твиттов (en,ru,ua...)
        "locale" => "ja",
        "result_type" => "recent", // тип твиттов,кот.мы хотим вытянуть(recent-последние; mixed-cмешанные; popular-популярные)
        "until" => "2015-07-25", // возвращаемые Твитты до указанной даты_(дата в формате:YYYY-MM-DD)
    ...
    // вставка полученных данных в foreach'е в табл.БД
        global $wpdb;
...
function update_table(){
    ...
    $content = $connection->get("search/tweets", array("count" => 5, //Поиск Твиттов в кол-ве("count" => 5),кот.мы вытягиваем к себе в ПЛАГИН
        ...
        "lang" => "en", // язык твиттов (en,ru,ua...)
        "locale" => "ja",
        "result_type" => "mixed", // тип твиттов,кот.мы хотим вытянуть(recent-последние; mixed-cмешанные; popular-популярные)
        "until" => "2015-07-25", // возвращаемые Твитты до указанной даты_(дата в формате:YYYY-MM-DD)
        "since_id" => "", // начальный № id (если надо выборка по номеру id)
        "max_id" => "", // конечный № id (если надо выборка по номеру id)
        "include_entities" => "false",
...
//echo '<pre>'; var_dump($content); die;echo '<pre>';// проверить что пришло
    ...
    if(empty($content->statuses)) { $wpdb->query("TRUNCATE TABLE " .DB_NAME.".".$table_name. ""); } //{ $wpdb->query('TRUNCATE TABLE  `wp_twitts`'); }
    else {
        // выбираем нужные нам значения из Объекта,в котором вытянутые Твиты и кот.пришел GET-запросом и ложим их в БД
        $cnt='';
        foreach ($content->statuses as $my_content) { // ->statuses - обращение к объекту в котором массив
...
} //END__function add_table
//add_action('wp_head', 'update_table'); // или на wp_head - закомментировано,т.к. мы эту функцию повесили на my_activation с ф-ей времени wp_schedule_event().Если надо,то расскоментировать это,-тогда данные по Твиттам будут добавляться в БД каждый раз при  обновлении страницы
//add_filter('the_content', 'add_table'); // НЕ работает(!)
...
add_action('my_hourly_event', 'update_table');
